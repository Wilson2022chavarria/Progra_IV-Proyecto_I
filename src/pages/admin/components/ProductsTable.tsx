    ...
    if (selectedProduct) {
      ...
      DataLayer.delete.product(selectedProduct.id!)
        .then(() => {
          ...
          const filteredProducts = products.filter((item: Product) => item.id !== selectedProduct.id);
          sessionStorage.setItem('products', JSON.stringify(filteredProducts));
          setListedProducts((prevState: Product[]) => prevState.filter((item: Product) => item.id !== selectedProduct.id));
        })
    ...
    if (selectedProduct) {
      ...
      if (p.id) {
        DataLayer.update.product(p)
          .then((editedProduct: Product) => {
            const productsString = sessionStorage.getItem('products') || '[]';
            const products: Product[] = JSON.parse(productsString);
            const updatedProducts = products.map((item: Product) => item.id === editedProduct.id ? editedProduct : item);
            sessionStorage.setItem('products', JSON.stringify(updatedProducts));
            setListedProducts((prevState: Product[]) => prevState.map((item: Product) => item.id === editedProduct.id ? editedProduct : item));
          })
          .catch((error: any) => setError(error))
          .finally(() => setLoading(false));
      } else {
        ...
        DataLayer.create.product(p)
          .then((createdProduct: Product) => {
            const productsString = sessionStorage.getItem('products') || '[]';
            const products: Product[] = JSON.parse(productsString);
            sessionStorage.setItem('products', JSON.stringify([...products, createdProduct]));
